...
/* their text:  var password = generatePassword();
  ...
  if (characterLength < 8 || characterLength > 128) {
    ...
    generateButton();
    return;
  /* 1st attempt, wonkier: if (characterLength < 8 || characterLength > 128) {
    alert("Error: password must be between 8-128 characters.")
    var characterLength = prompt("Enter length of password (8-128 characters).");
      if (characterLength < 8 || characterLength > 128) {
        alert("Error: password must be between 8-128 characters. Start over & refresh.")
        return; */
      };
  ...
  for (i=0; i < characterLength.length; i++) {
    var generatePassword = generatePassword.push(randomCharacter);

    //var generatePassword = generatePassword.push(randomCharacter + i+1)
  }
  console.log(generatePassword);


  // add password text to webpage
  document.getElementById('password').value='insert name of var for generated password here';
